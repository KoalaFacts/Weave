...
Current Agent actor keys remain `{workspaceId}/{agentName}`. `src/Invocations/InvokeTool/ToolActor.cs` now authorizes adapter-normalized `tool:<name>:invoke:<operation>` scopes separately from connection permission; the registry derives invocation grants from explicit Capabilities, not tool availability. The current host still uses Orleans and the existing Agent Runtime. Portable Agent identity, Tenant/Room authority, resource/installation revision constraints, generalized approval, and reconciliation remain target work. Read the [operation-authority migration record](docs/implementation/2026-09-19-exact-tool-operations.md); do not restore implicit grants or automatically broaden old `tool:<name>` entries.

ToolActor now requires durable invocation/attempt admission before dispatch. The host composes an on-disk SQLite journal, not an optional in-memory fallback. Owner-scoped outcome queries use `invocation:read`; duplicate submissions return metadata without replay. This is a single-host/preserved-file increment, not cluster-wide exactly-once execution, an approved-plan format, or a response-body cache. Read the [journal migration and limits](docs/implementation/2026-09-19-durable-invocations.md). Preserve unknown outcomes and retained IDs; do not add automatic replay, silent record deletion, or a production no-op journal.

The opt-in file-write approval profile adds protected pending plans, separate authorized reviewers, reject/cancel/expiry, and explicit resume through the existing ToolActor path. Pending plans have no Attempt. Approval is an extra obligation, not an execution grant; consume it atomically with attempt admission. Keep plan and ID serialization deterministic, revalidate before dispatch, and never accept replacement content on resume. This profile supports only bounded text writes in application-controlled roots, not other adapters or a production human identity flow. Preserve the adjacent approval protection keys with the database; see the [approval scope and migration notes](docs/implementation/2026-09-20-file-write-approval-v0.md).
