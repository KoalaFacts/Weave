...
impl ClientError {
    ...
    fn unconfirmed(kind: &'static str, id: &str) -> Self {
        Self {
            kind,
...
impl fmt::Display for ClientError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "{}; retain the original invocation ID. No automatic retry.",
            self.kind
        )
    }
}
...
impl Client {
    pub fn new(
        ...
        let mut credential =
            HeaderValue::from_str(capability).map_err(|_| ClientError::invalid())?;
        ...
        if let Some(bearer) = global_bearer {
            if bearer.is_empty()
                || bearer.len() > 16_384
                || !bearer.bytes().all(|c| (33..=126).contains(&c))
            {
                return Err(ClientError::invalid());
            }
    ...
    pub fn invoke(&self, request: &Invocation) -> Result<Reply, ClientError> {
        ...
        owned.invocation_id = canonical_id(&owned.invocation_id)?;
        if !valid_component(&owned.tool_name) || owned.method.trim().is_empty() {
            return Err(ClientError::invalid());
        }
    ...
    pub fn get_invocation(&self, tool: &str, id: &str) -> Result<Reply, ClientError> {
        ...
        self.send(tool, &id, &format!("/{id}"), None, false)
    }
    pub fn get_approval(&self, tool: &str, id: &str) -> Result<Reply, ClientError> {
        let id = canonical_id(id)?;
        self.send(tool, &id, &format!("/{id}/approval"), None, true)
    }
    fn send(
        &self,
        tool: &str,
        ...
        if !valid_component(tool) {
            return Err(ClientError::invalid());
        }
        let url = format!(
            "{}/api/workspaces/{}/tools/{tool}/invocations{suffix}",
        ...
        let response = request
            .send()
            .map_err(|_| ClientError::unconfirmed("transport", id))?;
        ...
        let is_json = response
            ...
            .unwrap_or(false);
        if !(200..600).contains(&status) || (300..400).contains(&status) || !is_json {
            return Err(ClientError::unconfirmed("protocol", id));
        }
...
fn validate_response(
    ...
    if value["invocationId"].as_str() != Some(id) {
        return Err(fail());
    }
    let states = [
        "Pending",
    ...
    if approval {
        if !value["approvalState"]
            .as_str()
            .is_some_and(|s| states.contains(&s))
        ...
        if !value["success"].is_boolean()
            ...
            || !value["duration"].is_string()
            || value["toolName"].as_str() != Some(tool)
            || !value["outcomeRecorded"].is_boolean()
            || !value["isReplay"].is_boolean()
        {
            return Err(fail());
        }
        if !value["outcome"].is_null()
            && !value["outcome"].as_str().is_some_and(|s| {
                [
                    "Succeeded",
                    "Failed",
                    "Denied",
        ...
        if value["success"] == true && value["outcome"] != "Succeeded" {
            return Err(fail());
        }
        if !value["approvalState"].is_null()
            && !value["approvalState"]
                .as_str()
                .is_some_and(|s| states.contains(&s))
        {
...
mod tests {
    ...
    fn shared_vectors_preserve_unicode_and_null_semantics() {
        let fixtures: Value = serde_json::from_str(include_str!(
            "../../../protocol/governed-tools/requests.json"
        ))
        .unwrap();
    ...
    fn invalid_ids_and_origins_reject_before_io() {
        for id in ["", "../other", "00000000000000000000000000000000"] {
            assert!(canonical_id(id).is_err());
        }
        for url in [
            "http://remote.example",
    ...
    fn unknown_or_incomplete_results_cannot_be_success() {
        assert!(
            validate_response(&serde_json::json!({"success":true}), "id", "files", false).is_err()
        );
    }
}
