...
fn run() -> Result<(), ClientError> {
    ...
    let result = match args[0].as_str() {
        "invoke" => {
            ...
            let request: Invocation =
                serde_json::from_slice(&bytes).map_err(|_| ClientError::invalid())?;
            if canonical_id(&request.invocation_id)? != id || request.tool_name != args[3] {
                return Err(ClientError::invalid());
            }
    ...
    println!(
        "{}",
        serde_json::to_string(&result).map_err(|_| ClientError::invalid())?
    );
    Ok(())
}
fn main() -> ExitCode {
    ...
    match run() {
        ...
        Err(error) => {
            eprintln!(
                "{}",
                serde_json::to_string(&error).unwrap_or_else(|_| "{\"kind\":\"protocol\"}".into())
            );
            ExitCode::from(2)
        }
    }
