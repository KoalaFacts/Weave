...
public sealed class ToolRegistryActorBranchTests
{
    ...
    public async Task GrantAgentToolsAsync_DeduplicatesAndReplaces()
    {
        ...
        await fx.Actor.GrantAgentToolsAsync("agent-a", ["t1", "t1", "t2"], ["tool:*"]);
        await fx.Actor.GrantAgentToolsAsync("agent-a", ["t3"], ["tool:*"]);
    ...
    public async Task ResolveAsync_ToolNotInAgentAccessList_ReturnsNull()
    {
        var fx = new Fixture();
        await fx.Actor.ConnectToolsAsync(SingleTool());
        await fx.Actor.GrantAgentToolsAsync("agent-a", ["different-tool"], ["tool:*"]);
    ...
    {
        var fx = new Fixture();
        await fx.Actor.GrantAgentToolsAsync("agent-a", ["ghost-tool"], ["tool:*"]);
    ...
    public async Task ResolveAsync_AllowedConnectedTool_ReturnsResolutionWithToken()
    {
        var fx = new Fixture();
        await fx.Actor.ConnectToolsAsync(SingleTool());
        await fx.Actor.GrantAgentToolsAsync("agent-a", ["shell"], ["tool:*"]);

        var result = await fx.Actor.ResolveAsync("agent-a", "shell");

        result.ShouldNotBeNull();
        result!.ToolName.ShouldBe("shell");
        result.Token.Grants.ShouldBe(["tool:shell:invoke:*"]);
