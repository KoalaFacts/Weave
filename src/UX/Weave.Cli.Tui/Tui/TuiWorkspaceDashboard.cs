...
internal sealed class TuiWorkspaceDashboard(GetSystemInfoAction systemInfoAction, IWorkspaceRegistry registry)
{
    ...
    private void AddWorkspaceRow(Table table, string name, string dir)
    {
        ...
        var statePath = manifestOk
            ? WorkspaceManifestPaths.GetStatePath(manifestPath)
            : null;
        ...
        table.AddRow(
            ...
            manifestOk
                ? StatusMarkup.ColorTag(CliTheme.Success, "● Ready")
                : StatusMarkup.ColorTag(CliTheme.Error, "✖ Missing"),
            hasState
                ? StatusMarkup.ColorTag(CliTheme.Info, "● Tracked")
                : StatusMarkup.ColorTag(CliTheme.Muted, "○ Idle"));
    }
