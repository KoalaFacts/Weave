...
public sealed class ProofVerifierActorTests
{
        ...
        Func<string, ProofOfWork, List<VerificationCondition>, string?, VerificationVote>? validatorBehavior = null)
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            var validatorActor = Substitute.For<IProofValidatorActor>();
            var capturedId = validatorId;
            validatorActor.ValidateAsync(capturedId, Arg.Any<ProofOfWork>(), Arg.Any<List<VerificationCondition>>(), Arg.Any<string?>())
                .Returns(callInfo =>
                {
    ...
    {
        var (verifier, _, agentActor, _) = CreateVerifier();
...
#pragma warning disable xUnit1051
        await agentActor.Received(1).ReviewTaskAsync(
            TestTaskId,
            true,
...
#pragma warning restore xUnit1051
    ...
    {
        var callCount = 0;
        var (verifier, _, agentActor, _) = CreateVerifier(validatorBehavior: (id, _, _, _) =>
        {
...
#pragma warning restore xUnit1051
    ...
    public async Task VerifyAsync_MajorityAccepts_WithThreeValidators_AcceptsTask()
    {
        ...
        var (verifier, _, agentActor, _) = CreateVerifier(state, validatorBehavior: (id, _, _, _) =>
        {
...
#pragma warning restore xUnit1051
    ...
    public async Task VerifyAsync_UsesConfiguredValidatorCount()
    {
        ...
        var (verifier, _, agentActor, _) = CreateVerifier(state);
        var proof = new ProofOfWork
        {
            Items = [new ProofItem { Type = ProofType.Custom, Label = "Note", Value = "done" }]
        };
