...
public sealed class ProofVerifierActor(
    ...
    [PersistentState("verifier", "Default")] IPersistentState<VerifierState> persistentState) : VirtualActor, IProofVerifierActor
{
    ...
    public async Task VerifyAsync(WorkspaceId workspaceId, string agentName, AgentTaskId taskId, ProofOfWork proof)
    {
        ...
        // Dispatch to N independent validator actors in parallel — each may use a different model
        ...
        for (var i = 0; i < validatorCount; i++)
        {
            ...
            var validatorActor = actors.GetActor<IProofValidatorActor>(VirtualActorId.From($"{workspaceId}/{validatorId}"));
            validationTasks.Add(validatorActor.ValidateAsync(validatorId, proof, conditions, modelId));
        }
