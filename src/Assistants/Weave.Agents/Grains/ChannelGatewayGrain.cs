...
public sealed class ChannelGatewayGrain(
    IGrainFactory grainFactory,
    IEventBus eventBus,
    ILogger<ChannelGatewayGrain> logger,
    [PersistentState("channel-gateway", "Default")] IPersistentState<ChannelGatewayState> persistentState) : Grain, IChannelGatewayGrain
{
    ...
    public async Task<OutboundMessage> RouteInboundAsync(InboundMessage message)
    {
        ...
        var agentGrain = grainFactory.GetGrain<IAgentGrain>($"{persistentState.State.WorkspaceId}/{agentName}");
        var response = await agentGrain.SendAsync(new AgentMessage
        {
            Role = "user",
