...
The Tools subsystem provides tool connectors, discovery, and actor-based tool execution with security scanning and lifecycle management.

## Projects

| Project | Purpose |
|---------|---------|
| `Weave.Tools` | Connectors, discovery service, tool actor, models, events |
| `Weave.Tools.Tests` | Tests for connectors, actor, discovery, security scanning |
...
**Key**: `{workspaceId}/{toolName}` — Orleans actor managing per-tool lifecycle.

### Interface

```csharp
public interface IToolActor : IVirtualActorWithStringKey
{
...
- **Secret substitution**: delegates to `ISecretProxyActor` for `{secret:path}` placeholder replacement.
...
- **Token validation** (ToolActor): invalid/expired tokens throw `UnauthorizedAccessException`
- **Secret scanning** (ToolActor): AWS keys blocked outbound, redacted inbound
