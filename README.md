...
The invocation ingress, verified review data and read-only Dashboard screen are available. The next user-facing milestone is real-browser acceptance and a separately authorized exact-plan decision workflow. It must not turn an unexplained digest, stale preview or self-issued identity into approval. Generalized target/resource constraints and safe recovery from uncertain results remain explicit work.
