...
internal static class ResumeInvocationEndpoint
{
        ...
        ICapabilityAuthorizer authorizer, IVirtualActorProvider actors)
    {
        ...
        try
        {
            var loaded = await new InvocationProposalReader(journal, authorizer).ReadForResumeAsync(workspaceId, toolName,
            ...
            var status = InvokeToolEndpoint.Status(result);
            if (status == 202)
            ...
            return Results.Json(InvocationHttpResult.FromResult(result), InvocationHttpJsonContext.Default.InvocationHttpResult,
                statusCode: status);
        }
